...
<% 
	...
	int firstDay = cal.get(Calendar.DAY_OF_WEEK);//요일 
	...
	int beforMonth = cal.get(Calendar.MONTH);
	cal.add(Calendar.MONTH, 2);//다음달 
	int nextYear = cal.get(Calendar.YEAR);
	int nextMonth = cal.get(Calendar.MONTH);
	cal.add(Calendar.MONTH, -1);//현재 원래 달 	
...
<body>
<a href="#" class="changeA" data-year="<%=beforYear%>" data-month="<%=beforMonth%>">이전달</a>
<h4><%=String.format("%tc",cal)%></h4>
<a href="#" class="changeA" data-year="<%=nextYear%>" data-month="<%=nextMonth%>">다음달</a>

<form action="Calendar.jsp" id="calForm">
...
<%
	...
	for(Locale tmp : locales){
		...
		tmp.toLanguageTag();
		out.println(
				String.format("<option value='%s'>%s</option>", value, selected, text)
				);
	}
%>
</select>
...
<%
	...
	for(int row=1; row<=6; row ++){		
		...
		for(int column=1; column<=7; column++){
			 ...
			 if(column ==1){
				  out.println(	
				  String.format("<td class='red'>%s</td>",dateStr)
			   );
			  }		 
			 else if(column ==7){
				 out.println(	
				   String.format("<td class='blue'>%s</td>",dateStr)
				 );
			 }else{
			 out.println(					 
