 ...
 *  
 *  서블릿 개발 단계
 *  1. HttpServlet을 상속받은 클래스 정의
 *  2. 컴파일 -> context의 classpath(/WEB-INF/classes)에 배포.
 *  3. 서블릿 컨테이너에 등록
 *  	1) web.xml(deployment descriptor)
 *  		servlet -> servlet-name, servlet-class
 *  	2) @WebServlet(name..) : servlet 3.0부터
 ...
 *  
 *  servlet container ? 서블릿의 라이프사이클에 대한 관리자.
 *  
 *  callback $("button").on("click", function(event){alert});
 *  	: 특정 조건(event)이 발생했을 때 시스템 내부적으로 호출되는 구조
 *  
...
public class DescriptionServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		super.service(req, resp); // request method에 따라 do게열의 콜백을 호출하는 역할
